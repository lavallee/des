...
1. **Research the task, not a style.** Use local product evidence first. Query
   Mobbin for a specific screen or flow only when an outside interaction pattern
   could answer a named question.
2. **Capture the current route.** Run `des-audit` at 1440, 768, and 390 px before
   changing it. Keep the receipt as the baseline arm.
3. **Choose whether divergence is useful.** For a genuinely new direction, make
   candidates that differ on one named axis. Image generation is optional and
   stays upstream of implementation.
4. **Implement in the owning application.** Reconstruct the chosen mechanism
   with real HTML, CSS, components, states, and data. Never cut a generated image
   into interface slices.
5. **Exercise the representative task.** Use stable role/name selectors for known
...
6. **Reshoot and diagnose.** Run `des-audit` again. If it finds a performance or
...
7. **Promote with authority.** An independent seeing pass or explicit human
...
The runner writes a full-page PNG and compact accessibility snapshot for each
viewport plus a JSON receipt. It fails on blank renders, failed images, body
...
budgets. The focus check asks Chromium to force keyboard-modality focus so
`:focus-visible` rules are evaluated rather than inferred from pointer modality.
